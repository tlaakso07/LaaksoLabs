...
export function GoogleConnectWizard({ onClose }: Props) {
  const [redirectUri] = useState(() =>
    typeof window !== 'undefined'
      ? `${window.location.origin}/api/campaigns/google/oauth-callback`
      : '',
  )
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const res = await fetch(`/api/campaigns/google/oauth-status/${oauthState}`, { credentials: 'include' })
  ...
  async function startAuth() {
    ...
    setErrorMsg('')
    try {
      const res = await fetch('/api/campaigns/google/auth-start', {
        method: 'POST',
        credentials: 'include',
                ...
                },
                {
                  ...
                  extra: (
                    <div style={{ marginTop: '8px', display: 'flex', gap: '6px', alignItems: 'center' }}>
                      <code style={{ flex: 1, padding: '7px 10px', borderRadius: '7px', background: 'var(--surface-2)', border: '1px solid var(--border)', fontSize: '11px', color: 'var(--text)', wordBreak: 'break-all' }}>
                        {redirectUri || 'Loading...'}
                      </code>
          ...
          {step === 'credentials' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0, lineHeight: 1.6 }}>
                Enter the credentials you collected. They&apos;re only held server-side until the OAuth flow completes.
              </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 16px', borderRadius: '10px', background: '#0d2e1a', border: '1px solid #1a5c35' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '12px', color: '#4ade80', margin: 0 }}>The tab that opened shows all 5 secrets to copy.</p>
                </div>
              </div>
              ...
              <div style={{ padding: '14px 16px', borderRadius: '10px', background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
                ...
                <ol style={{ paddingLeft: '18px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {[
                    ...
                    'Open Vercel dashboard → laaksolabs → Settings → Environment Variables',
                    'Add all 5 variables for Production with the exact names shown',
              ...
              <div style={{ padding: '12px 16px', borderRadius: '8px', background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
                <p style={{ fontSize: '11px', color: 'var(--text-faint)', margin: '0 0 8px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Variables needed</p>
