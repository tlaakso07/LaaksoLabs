...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const currentMRR = (revenues ?? []).filter(r => (r as { month: string }).month === currentMonth).reduce((s: number, r) => s + ((r as { amount: number }).amount ?? 0), 0)
      const months: string[] = []
      for (let i = 5; i >= 0; i--) { const d = new Date(); d.setMonth(d.getMonth() - i); months.push(getFirstOfMonth(d)) }
      const chartData = months.map(m => {
        const me = (revenues ?? []).filter(r => (r as { month: string }).month === m)
      ...
      setData({ clients: clientList, openTasks: taskList, recentActivity: (activity ?? []) as DashboardData['recentActivity'], chartData, currentMRR })
    }
    load()
  }, [])
  ...
  const activeClients = { div1: data.clients.filter(c => c.status === 'active' && c.division === 'div1').length, div2: data.clients.filter(c => c.status === 'active' && c.division === 'div2').length, div3: data.clients.filter(c => c.status === 'active' && c.division === 'div3').length }
  ...
  const prioritizedTasks = [...data.openTasks].filter(t => t.due_date && (isDueToday(t.due_date) || isOverdue(t.due_date))).sort((a, b) => {
