...
interface ActivityEntry { id: string; type: string; content: string; created_at: string; clients?: { name: string } | null }

const ICON_MAP: Record<string, React.ReactNode> = {
  call: <Phone size={10} />, email: <Mail size={10} />, note: <FileText size={10} />,
  meeting: <Users size={10} />, milestone: <Star size={10} />, payment: <DollarSign size={10} />,
}
const TYPE_COLOR: Record<string, string> = {
  call: 'var(--div1)', email: 'var(--div2)', note: 'var(--text-muted)',
  meeting: 'var(--div3)', milestone: 'var(--high)', payment: 'var(--active)',
}
    ...
    <div>
      ...
      {entries.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '32px 0' }}>
          ...
          <p style={{ fontSize: '11px', color: 'var(--text-faint)', marginTop: '4px' }}>Log calls, emails, and milestones from client pages</p>
        </div>
      ) : (
        <div>
          {entries.map((entry, i) => {
