  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/clients" className="flex items-center gap-1 cursor-pointer" style={{ fontSize: '12px', color: 'var(--text-muted)', textDecoration: 'none', transition: 'color 120ms ease' }}
      ...
      {visible.length === 0 ? (
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
          {visible.map((client, i) => {
            ...
            return (
              <Link key={client.id} href={`/clients/${client.id}`} className="animate-in card card-hover block cursor-pointer rounded-xl"
                style={{ animationDelay: `${i * 55}ms`, textDecoration: 'none', padding: '20px', borderTop: `2px solid color-mix(in srgb, ${divColor} 50%, transparent)` }}
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  {client.retainer_amount
                    ? <span className="num font-semibold" style={{ fontSize: '13px', color: 'var(--text)' }}>{formatCurrency(client.retainer_amount)}<span style={{ fontSize: '10px', fontWeight: 400, color: 'var(--text-muted)' }}>/mo</span></span>
