  ...
  return (
    ...
    >
      ...
      <nav style={{ flex: 1, padding: '12px', overflowY: 'auto' }}>
        <p style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--sidebar-text-faint)', padding: '0 10px', marginBottom: '8px' }}>
          Navigation
        </p>
          ...
          return (
            <Link
              ...
              style={{
                ...
                borderRadius: '8px', fontSize: '13px', fontWeight: active ? 500 : 400,
                color: active ? 'var(--sidebar-text)' : 'var(--sidebar-text-dim)',
                background: active ? 'var(--sidebar-active)' : 'transparent',
                borderLeft: `2px solid ${active ? 'var(--sidebar-accent)' : 'transparent'}`,
                textDecoration: 'none', transition: 'background 100ms ease, color 100ms ease',
                marginBottom: '2px',
              }}
              onMouseEnter={e => {
                if (!active) {
            ...
            >
              <Icon size={14} strokeWidth={active ? 2 : 1.5} style={{ color: 'var(--sidebar-accent)', flexShrink: 0 }} />
              {label}
            </Link>
          )
        })}
      ...
      {/* MRR Progress */}
      <div style={{ margin: '12px', padding: '16px', borderRadius: '12px', background: 'var(--sidebar-card)', border: '1px solid var(--sidebar-border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
          <span style={{ fontSize: '10px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--sidebar-text-dim)' }}>MRR Target</span>
          <span className="num" style={{ fontSize: '11px', color: 'var(--sidebar-accent)', fontWeight: 600 }}>{pct.toFixed(1)}%</span>
        </div>
        <div style={{ height: '3px', borderRadius: '2px', background: 'var(--sidebar-track)', overflow: 'hidden', marginBottom: '10px' }}>
          <div style={{ height: '100%', borderRadius: '2px', width: `${pct}%`, background: 'var(--sidebar-accent)', transition: 'width 1.2s cubic-bezier(0.16,1,0.3,1)' }} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span className="num" style={{ fontSize: '13px', color: 'var(--sidebar-text)', fontWeight: 600 }}>{formatCurrency(currentMRR)}</span>
          <span className="num" style={{ fontSize: '11px', color: 'var(--sidebar-text-faint)' }}>{gap > 0 ? `${formatCurrency(gap)} to go` : 'Target met'}</span>
        </div>
      </div>
      ...
      <button
        onClick={async () => {
          ...
          await logout()
          window.location.href = window.location.pathname.replace(/\/$/, '') + '/login' + window.location.search
        }}
        style={{
          display: 'flex', alignItems: 'center', gap: '10px',
          margin: '0 12px 14px', padding: '9px 12px', borderRadius: '8px',
          fontSize: '12px', fontWeight: 500, color: 'var(--sidebar-text-dim)',
