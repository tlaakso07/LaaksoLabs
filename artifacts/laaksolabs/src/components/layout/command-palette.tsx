  ...
  return (
    ...
    >
      <div style={{
        ...
        borderRadius: '16px', boxShadow: 'var(--shadow-elevated)', overflow: 'hidden',
      }}>
        <div style={{
          ...
          <input
            ...
            style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontSize: '15px', color: 'var(--text)', fontFamily: 'inherit' }}
          />
          <kbd style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '4px', background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text-muted)', fontFamily: 'inherit' }}>ESC</kbd>
        </div>
        {results.length > 0 && (
          <div style={{ maxHeight: '360px', overflowY: 'auto', padding: '8px' }}>
            {groups.map(group => (
              <div key={group.label}>
                <p style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-faint)', padding: '6px 10px 4px' }}>{group.label}</p>
                {group.items.map(item => {
                  ...
                  return (
                    ...
                    >
                      {item.kind === 'page' && <item.icon size={14} strokeWidth={1.5} style={{ color: 'var(--accent)', flexShrink: 0 }} />}
                      {item.kind === 'client' && <div style={{ width: '14px', height: '14px', borderRadius: '50%', background: 'var(--surface-2)', border: '1px solid var(--border)', flexShrink: 0 }} />}
                      {item.kind === 'task' && <div style={{ width: '8px', height: '8px', borderRadius: '50%', flexShrink: 0, background: TASK_PRIORITY_COLORS[item.priority] }} />}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.label}</p>
                        {item.description && <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '1px' }}>{item.description}</p>}
                      </div>
                    </button>
                  )
                })}
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '8px 16px', display: 'flex', gap: '16px' }}>
